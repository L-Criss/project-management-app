        ...
        <table className="w-full text-sm text-left rtl:text-right text-gray-500">
          <thead className="text-xs text-gray-700 uppercase bg-gray-50 border-b-2 border-gray-500">
            <tr className="text-nowrap">
              <TableHeading name="id" sort_field={queryParams.sort_field} sort_order={queryParams.sort_order} sortChange={sortChange}>ID</TableHeading>
              <th className="px-3 py-3">Image</th>
              {!hideProjectName && <th className="px-3 py-3">Project Name</th>}
              <TableHeading name="name" sort_field={queryParams.sort_field} sort_order={queryParams.sort_order} sortChange={sortChange}>Name</TableHeading>
              <TableHeading name="status" sort_field={queryParams.sort_field} sort_order={queryParams.sort_order} sortChange={sortChange}>Status</TableHeading>
              <TableHeading name="created_at" sort_field={queryParams.sort_field} sort_order={queryParams.sort_order} sortChange={sortChange}>Create Date</TableHeading>
              <TableHeading name="due_date" sort_field={queryParams.sort_field} sort_order={queryParams.sort_order} sortChange={sortChange}>Due Date</TableHeading>
            ...
            {tasks.data.map(task => (
              <tr key={task.id} className="bg-white border-b">
                ...
                <td className="px-3 py-2">
                  {task.image_path && <img src={task.image_path} alt={task.name} style={{width: 60}} />}
                </td>
                ...
                <td className="px-3 py-2 text-nowrap">
                  <Link href={route('tasks.edit', task.id)} className="font-medium text-blue-600 hover:underline mx-1">Edit</Link>
                  <button onClick={() => deleteTask(task)} className="font-medium text-red-600 hover:underline mx-1">Delete</button>
                </td>
              </tr>
            ))}
          </tbody>
